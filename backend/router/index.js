...
const { Data,SignConfig,CardKey,Plan,User } = require('@/database/models');
const { format } = require('@fast-csv/format');
const logger = require('@/utils/logger');
const { generateCardKeys, redeemCardKey } = require('@/utils/cardkey');
const botWorker = getBotWorker();
...
module.exports = async function (app) {
    ...
    app.get('/status', async (req, reply) => {
        ...
        const running = !!(botWorker && botWorker.threadId && botWorker.exitCode === null);
        reply.send({ running });
    });
    ...
    app.get('/signconfig', async (req, reply) => {
        const config = await SignConfig.findOne();
        reply.send(config || {});
    });

    app.post('/signconfig', async (req, reply) => {
    ...
    app.post('/method', async (req, reply) => {
        const { name, display_name, description, status, slot_limit, point } = req.body;
        ...
        try {
            ...
            const record = await Method.create({
                ...
                point: typeof point === 'number' ? point : 0, // 兼容默认值
            });
    ...
    app.post('/method/:id/point', async (req, reply) => {
    ...
    app.post('/method/:id', async (req, reply) => {
        const id = req.params.id;
        const fields = req.body; // point, slot_limit, display_name...
        try {
            const Method = require('@/database/models/Method');
            const method = await Method.findByPk(id);
            if (!method) return reply.code(404).send({ error: '方法未找到' });
    ...
    // 更新 API 绑定的方法
    app.post('/api/:id/methods', async (req, reply) => {
        const id = req.params.id;
    ...
    // 获取所有方法
    app.get('/method/list', async (req, reply) => {
        const list = await require('@/database/models/Method').findAll();
        reply.send(list);
    });

    // 获取 API 已绑定方法
    app.get('/api/:id/methods', async (req, reply) => {
        const Api = require('@/database/models/Api');
        const Method = require('@/database/models/Method');
    ...
    app.post('/plan/:id', async (req, reply) => {
        const id = req.params.id;
    ...
    // 删除套餐
    app.post('/plan/:id/delete', async (req, reply) => {
        const id = req.params.id;
        try {
            const Plan = require('@/database/models/Plan');
            const plan = await Plan.findByPk(id);
    ...
    // 套餐列表
    app.get('/plan/list', async (req, reply) => {
        const Plan = require('@/database/models/Plan');
        const list = await Plan.findAll();
        reply.send(list);
    });

    app.post('/cardkey/generate', async (req, reply) => {
    ...
    app.get('/cardkey/list', async (req, reply) => {
    ...
    app.get('/cardkey/export', async (req, reply) => {
    ...
    // 4. 修改用户
    app.post('/user/:id/update', async (req, reply) => {
        const user = await User.findByPk(req.params.id);
        if (!user) return reply.code(404).send({ error: '用户不存在' });
    ...
    // 5. 封禁/解封
    app.post('/user/:id/ban', async (req, reply) => {
        const user = await User.findByPk(req.params.id);
        if (!user) return reply.code(404).send({ error: '用户不存在' });
        user.status = 'banned';
        await user.save();
    ...
    app.post('/user/:id/unban', async (req, reply) => {
        const user = await User.findByPk(req.params.id);
        if (!user) return reply.code(404).send({ error: '用户不存在' });
        user.status = 'active';
        await user.save();
    ...
    // 6. 积分增减
    app.post('/user/:id/point', async (req, reply) => {
    ...
    // 7. 删除用户
    app.post('/user/:id/delete', async (req, reply) => {
        const user = await User.findByPk(req.params.id);
        if (!user) return reply.code(404).send({ error: '用户不存在' });
        await user.destroy();
        reply.send({ success: true });
    ...
    // 8. 列出所有套餐（下拉用）
    app.get('/user/plan-list', async (req, reply) => {
