...
const { SignConfig } = require('@/database/models');
...
const commands = {
    hello: {
        ...
        adminOnly: false,
        async execute(ctx) {
            await ctx.reply('Powered By @Core_888');
        }
    },
    ...
    attack: {
        ...
        async execute(ctx, args) {
            ...
            logger.warn(`攻击任务: target=${target}, port=${port || 'N/A'}, time=${time}, method=${method}, slot=${slot}`);
